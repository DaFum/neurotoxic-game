...
**The scenes render correctly and on-brand. Two real defects were found and fixed** — overworld node-label collisions and the chatter box visually overlapping interactive UI. Everything else is a sandbox image-CDN artifact. The per-scene table below records the _post-fix_ state. Details follow.
...
| **POSTGIG**         | ✅ (see note) | Under state-injection shows the report _shell_ ("TALLYING RECEIPTS…"); the figures are computed by the live END_GIG flow, so this is an injection limitation, **not** a scene bug. |
