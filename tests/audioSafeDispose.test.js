...
import { safeDispose } from '../src/utils/audio/setup.js'
...
test('safeDispose', async t => {
  ...
  await t.test('swallows error, logs it, and returns null when dispose() throws', () => {
    ...
    try {
      ...
      assert.ok(lastCall.arguments[1].includes('Node disposal failed'), 'Log message should contain expected text')
