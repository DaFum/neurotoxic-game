...
describe('Relationship Events Data', () => {
  ...
  it('evaluates conditions correctly based on relationships', () => {
    const toxicInfighting = RELATIONSHIP_EVENTS.find(
      e => e.id === 'toxic_infighting'
    )
    const synergyMoment = RELATIONSHIP_EVENTS.find(
      e => e.id === 'synergy_moment'
    )
    ...
    assert.deepStrictEqual(toxicInfighting.condition(lowRelState), {
      member1: 'Alice',
      member2: 'Bob'
    })
    ...
    assert.deepStrictEqual(synergyMoment.condition(highRelState), {
      member1: 'Alice',
      member2: 'Bob'
    })
