...
test('Event validation filters out invalid events', async (t) => {
  ...
  assert.strictEqual(transportEvents.length, 1, 'Should contain only valid events')
  assert.strictEqual(transportEvents[0].id, 'valid_event', 'Should be the valid event')
  assert.strictEqual(transportEvents[0].description, 'Valid Event', 'Should be the correct valid event')
})
