...
describe('SPECIAL_EVENTS data module', () => {
  ...
  it('each event should follow the strict schema', () => {
    ...
    for (const event of SPECIAL_EVENTS) {
      ...
      assert(
        typeof event.description === 'string' && event.description.startsWith('events:'),
        `Event ${event.id} description must be an i18n key starting with 'events:'`
      )

      assert(Array.isArray(event.options), `Event ${event.id} options must be an array`)
      assert(event.options.length > 0, `Event ${event.id} must have at least one option`)

      for (const option of event.options) {
        assert(
          typeof option.label === 'string', // && option.label.startsWith('events:'),
          `Event ${event.id} option label must be an i18n key`
        )
        assert(
          typeof option.outcomeText === 'string', // && option.outcomeText.startsWith('events:'),
        ...
        if (option.effect) {
          const validTypes = ['stat', 'item', 'resource', 'composite']
          assert(
          ...
          if (option.effect.type === 'composite') {
            assert(Array.isArray(option.effect.effects), 'composite effect requires effects array')
          }
        }

        if (option.skillCheck) {
          assert(typeof option.skillCheck.stat === 'string', `Event ${event.id} skillCheck stat invalid`)
          assert(typeof option.skillCheck.threshold === 'number', `Event ${event.id} skillCheck threshold invalid`)
          assert(typeof option.skillCheck.success === 'object', `Event ${event.id} skillCheck success effect invalid`)
          assert(typeof option.skillCheck.failure === 'object', `Event ${event.id} skillCheck failure effect invalid`)
        }
      }
    }
  })
