...
describe('travelUtils', () => {
  ...
  describe('calculateTravelCostsAndImpact', () => {
    ...
    test('calculates correct travel costs and impact based on mocks and forwards correct inputs', () => {
      ...
      assert.strictEqual(
        vi.mocked(calculateTravelExpenses).mock.calls.length,
        1
      )
      ...
      assert.strictEqual(
        vi.mocked(getTotalDailyObligations).mock.calls.length,
        1
      )
      assert.deepStrictEqual(
        vi.mocked(getTotalDailyObligations).mock.calls[0],
        [
          {
            player: mockPlayer,
            band: mockBand,
