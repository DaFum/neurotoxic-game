...
const createMockSetters = gameStateRef => ({
  setScore: vi.fn(updater => { const next = typeof updater === 'function' ? updater(gameStateRef.current.score) : updater; gameStateRef.current.score = next; return next; }),
  setCombo: vi.fn(updater => { const next = typeof updater === 'function' ? updater(gameStateRef.current.combo) : updater; gameStateRef.current.combo = next; return next; }),
  setHealth: vi.fn(updater => { const next = typeof updater === 'function' ? updater(gameStateRef.current.health) : updater; gameStateRef.current.health = next; return next; }),
  setOverload: vi.fn(updater => { const next = typeof updater === 'function' ? updater(gameStateRef.current.overload) : updater; gameStateRef.current.overload = next; return next; }),
  setIsToxicMode: vi.fn(val => { gameStateRef.current.isToxicMode = val }),
  setIsGameOver: vi.fn(val => { gameStateRef.current.isGameOver = val }),
  setAccuracy: vi.fn(updater => { const next = typeof updater === 'function' ? updater(gameStateRef.current.accuracy) : updater; gameStateRef.current.accuracy = next; return next; })
})
...
describe('useRhythmGameScoring', () => {
  ...
  beforeEach(async () => {
    ...
    contextActions = { addToast: vi.fn(), setLastGigStats: vi.fn(), endGig: vi.fn() }

    mockAudioEngine.getGigTimeMs.mockImplementation(() => 1000)
    mockRhythmUtils.checkHit.mockImplementation(() => ({ hit: false, visible: true, time: 1000, originalNote: { p: 60 } }))
  ...
  test('initializes and handles various hit/miss scenarios efficiently', () => {
    ...
    expect(contextActions.addToast.mock.calls[0][0]).toBe('ui:gig.toasts.bandCollapsed')
  ...
  test('handles toxic mode activation, scoring, and deactivation', () => {
    ...
    expect(contextActions.addToast.mock.calls[0][0]).toBe('ui:gig.toasts.toxicModeLost')
