...
vi.mock('react-i18next', () => ({
  ...
  useTranslation: () => ({
    t: (key, options) => {
      ...
      if (key === 'report.amount_positive') return `+€${options.amount}`
      if (key === 'report.amount_negative') return `-€${options.amount}`
      if (key === 'report.amount_with_currency') return `€${options.amount}`
      return key
    },
    i18n: {
      language: 'en',
