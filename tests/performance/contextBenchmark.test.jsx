...
test('GameStateProvider Re-render Benchmark', async (t) => {
  ...
  const { GameStateProvider, useGameState, useGameDispatch } = await import('../../src/context/GameState.jsx');
  ...
  assert.strictEqual(consumerRenders, 1, 'Consumer should re-render after state update');

  // Trigger component also re-renders because useGameState returns new object
  assert.strictEqual(triggerRenders, 1, 'Unoptimized Trigger should re-render after state update');

  // Optimized Trigger should NOT re-render because dispatch context is stable
  assert.strictEqual(optimizedRenders, 0, 'Optimized Trigger should NOT re-render');
});
