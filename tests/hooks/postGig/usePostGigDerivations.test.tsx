...
import type { Venue, PlayerState, BandState, SocialState, GigStats, CityTraitState } from '../../../src/types'
...
describe('usePostGigDerivations', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(postGigUtils.deriveGigContext).mockReturnValue({ daysSinceLastGig: 1, lastGigDifficulty: null })
    vi.mocked(postGigUtils.deriveFinancials).mockReturnValue({ income: { total: 100 }, expenses: { total: 0 }, net: 100 } as any)
    vi.mocked(postGigUtils.derivePostOptions).mockReturnValue({ options: [{ type: 'rest' }], error: null } as any)
    ...
    vi.mocked(mapGenerator.deriveCityTraits).mockReturnValue({ tech_hub: true } as any)
  ...
  it('computes correct memos on render', () => {
    ...
    expect(result.current.financials).toEqual({ income: { total: 100 }, expenses: { total: 0 }, net: 100 })
    ...
    expect(postGigUtils.deriveGigContext).toHaveBeenCalledWith(defaultProps.currentGig, defaultProps.social, defaultProps.player)
    expect(assetSelectors.getActiveAssetModifiers).toHaveBeenCalledWith(defaultProps.assets)
    expect(mapUtils.normalizeVenueId).toHaveBeenCalledWith('test-venue')
    expect(mapGenerator.getCityKeyFromVenueId).toHaveBeenCalledWith('normalized-venue')
    expect(mapGenerator.deriveCityTraits).toHaveBeenCalledWith('test-city')

    expect(postGigUtils.deriveFinancials).toHaveBeenCalledWith(expect.objectContaining({
    ...
    expect(postGigUtils.derivePostOptions).toHaveBeenCalledWith(expect.objectContaining({
      currentGig: defaultProps.currentGig,
      lastGigStats: defaultProps.lastGigStats,
  ...
  it('does not trigger events if currentGig is null', () => {
    renderHook(() => usePostGigDerivations({ ...defaultProps, currentGig: null }))
    expect(mockTriggerEvent).not.toHaveBeenCalled()
  })

  it('does not trigger events if activeEvent is present', () => {
    renderHook(() => usePostGigDerivations({ ...defaultProps, activeEvent: { type: 'some_event', id: 'some' } as any }))
    expect(mockTriggerEvent).not.toHaveBeenCalled()
  })
  ...
  it('uses cityStates from props if available instead of deriving', () => {
    const cityStates = { 'test-city': { tech_hub: false } as any }
