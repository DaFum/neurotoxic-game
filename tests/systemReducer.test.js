...
test('systemReducer - LOAD_GAME', async t => {
  await t.test('loads game and sanitizes player, band, and social state', () => {
    ...
    assert.deepEqual(nextState.band.members[0], { id: 'm1', mood: 80, stamina: 70, traits: ['trait1'] })
    ...
    assert.deepEqual(nextState.toasts, [{ id: '1', message: 'Hello', type: 'info' }])
  ...
  await t.test('handles missing or malformed loaded state gracefully', () => {
    ...
    const loadedState = {
      ...
      toasts: [{ id: '1' }, { id: '2', message: '  ', type: 'invalid' }, { id: '3', message: 'Valid' }],
      setlist: 'invalid',
    ...
    assert.deepEqual(nextState.toasts, [{ id: '3', message: 'Valid', type: 'info' }])
  ...
  await t.test('handles missing or malformed toasts array', () => {
    const initialState = createInitialState()
    const loadedState = {
      toasts: [{ id: '1' }, null, { id: '2', message: '  ', type: 'invalid' }, { id: '3', message: 'Valid', type: 'error' }]
    }
...
test('systemReducer - RESET_STATE', async t => {
  ...
  await t.test('falls back to current settings and empty unlocks if payload is empty', () => {
    const initialState = createInitialState()
    const currentState = {
      ...initialState,
      player: { ...initialState.player, money: 9999 },
...
test('systemReducer - ADVANCE_DAY core logic', async t => {
  await t.test('processes daily updates, resets event count, and handles scandal flag', () => {
    const initialState = createInitialState()
