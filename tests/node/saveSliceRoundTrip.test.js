...
const buildPopulatedState = () => {
  ...
  state.pendingRiskEvent = { assetId: 'asset_1', kind: 'tourbus_chassis' }
  ...
  state.liabilities = [
    { id: 'liability_1', assetId: 'asset_1', principal: 3000, ratePerDay: 0.01 }
  ]
  state.crowdfundCampaigns = [
    { id: 'campaign_1', assetKind: 'tourbus_chassis', goal: 5000, pledged: 900 }
  ]
  state.rngSeed = 123456
  state.runSeed = 654321
  state.rivalBand = { id: 'rival_1', name: 'Kranker Schrank', fame: 120 }
...
describe('persisted save slice round-trip', () => {
  ...
  it('a reloaded save re-serializes to the same slices', () => {
    ...
    for (const slice of sliceNames) {
      assert.strictEqual(
        typeof serialize(reserialized[slice]),
        'string',
        `${slice} is not serializable after a load`
      )
    }
  })
})
