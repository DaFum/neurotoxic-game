...
test('advanceQuest', async t => {
  ...
  await t.test('returns original state if required is missing or invalid', () => {
    const state = { activeQuests: [{ id: 'q1', progress: 0 }] }
    const nextState = QuestLifecycle.advanceQuest(state, { questId: 'q1' })
    assert.equal(nextState, state)

    const stateInvalidRequired = { activeQuests: [{ id: 'q1', required: -5, progress: 0 }] }
    const nextStateInvalidRequired = QuestLifecycle.advanceQuest(stateInvalidRequired, { questId: 'q1' })
  ...
  await t.test('returns original state if amount is not a number', () => {
    ...
    const nextState = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: 'abc' })
    assert.equal(nextState, state)
  })

  await t.test('returns original state if amount is negative or non-finite', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextStateNeg = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: -1 })
    assert.equal(nextStateNeg, state)

    const nextStateInf = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: Infinity })
    assert.equal(nextStateInf, state)

    const nextStateNaN = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: NaN })
  ...
  await t.test('advances quest progress with specified amount', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextState = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: 3 })
    assert.notEqual(nextState, state)
  ...
  await t.test('advances quest progress with amount 0', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextState = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: 0 })
    assert.notEqual(nextState, state)
  ...
  await t.test('completes quest when progress reaches required', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 3, label: 'Q' }], toasts: [] }
    const nextState = QuestLifecycle.advanceQuest(state, { questId: 'q1', amount: 2, randomIdx: 42 })
...
test('setQuestProgress', async t => {
  await t.test('returns original state if activeQuests is missing', () => {
    const state = {}
    const nextState = QuestLifecycle.setQuestProgress(state, { questId: 'q1', progress: 5 })
    assert.equal(nextState, state)
  })
  ...
  await t.test('does not lower progress if new progress is lower', () => {
    ...
    const nextState = QuestLifecycle.setQuestProgress(state, { questId: 'q1', progress: 2 })
    assert.equal(nextState.activeQuests[0].progress, 5)
  })

  await t.test('caps progress at required and completes quest', () => {
    const state = { activeQuests: [{ id: 'q1', progress: 8, required: 10, label: 'Q' }], toasts: [] }
    const nextState = QuestLifecycle.setQuestProgress(state, { questId: 'q1', progress: 15 })
    assert.equal(nextState.activeQuests.length, 0)
  })
  ...
  await t.test('updates progress even if required is missing (threshold-less)', () => {
