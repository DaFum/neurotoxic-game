...
import { advanceQuest, setQuestProgress } from '../../../src/domain/questAdvance.ts'

test('advanceQuest', async t => {
  await t.test('returns original state if activeQuests is missing', () => {
    const state = {}
    const nextState = advanceQuest(state, { questId: 'q1' })
  ...
  await t.test('returns original state if required is missing or invalid', () => {
    ...
    const nextStateInvalidRequired = advanceQuest(stateInvalidRequired, { questId: 'q1' })
  ...
  await t.test('returns original state if amount is not a number', () => {
    ...
    const nextState = advanceQuest(state, { questId: 'q1', amount: 'abc' })
    assert.equal(nextState, state)
  })

  await t.test('returns original state if amount is negative or non-finite', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextStateNeg = advanceQuest(state, { questId: 'q1', amount: -1 })
    assert.equal(nextStateNeg, state)

    const nextStateInf = advanceQuest(state, { questId: 'q1', amount: Infinity })
    assert.equal(nextStateInf, state)

    const nextStateNaN = advanceQuest(state, { questId: 'q1', amount: NaN })
  ...
  await t.test('advances quest progress with specified amount', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextState = advanceQuest(state, { questId: 'q1', amount: 3 })
    assert.notEqual(nextState, state)
  ...
  await t.test('advances quest progress with amount 0', () => {
    const state = { activeQuests: [{ id: 'q1', required: 5, progress: 0 }] }
    const nextState = advanceQuest(state, { questId: 'q1', amount: 0 })
    assert.notEqual(nextState, state)
  ...
  await t.test('completes quest when progress reaches required', () => {
    ...
    const nextState = advanceQuest(state, { questId: 'q1', amount: 2, randomIdx: 42 })
...
test('setQuestProgress', async t => {
  await t.test('returns original state if activeQuests is missing', () => {
    const state = {}
    const nextState = setQuestProgress(state, { questId: 'q1', progress: 5 })
    assert.equal(nextState, state)
  })
  ...
  await t.test('does not lower progress if new progress is lower', () => {
    ...
    const nextState = setQuestProgress(state, { questId: 'q1', progress: 2 })
    assert.equal(nextState.activeQuests[0].progress, 5)
  })
  ...
  await t.test('caps progress at required and completes quest', () => {
    ...
    const nextState = setQuestProgress(state, { questId: 'q1', progress: 15 })
    assert.equal(nextState.activeQuests.length, 0)
  })
