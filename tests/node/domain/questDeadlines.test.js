...
test('checkDeadlines', async t => {
  ...
  await t.test('removes expired quest, adds failure flags, toasts and cooldowns, removes startFlags', () => {
    const state = {
      player: { day: 10 },
    ...
    assert.deepEqual(nextState.questCooldowns, [{ questId: 'q1', expiresOnDay: 13 }])
  ...
  await t.test('preserves toasts and cooldowns arrays when pushing to them', () => {
    const state = {
      player: { day: 10 },
  ...
  await t.test('handles flagsToAdd with new flags without altering existing that are not to remove', () => {
    const state = {
      player: { day: 10 },
