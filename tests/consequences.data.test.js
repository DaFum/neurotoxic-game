...
describe('Consequences Event Pool', () => {
  ...
  it('evaluates conditions properly', () => {
    ...
    assert.strictEqual(venueComplaint.condition({ lastGigStats: { score: 20 }, eventCooldowns: [] }), true);
    assert.strictEqual(venueComplaint.condition({ lastGigStats: { score: 40 }, eventCooldowns: [] }), false);
    assert.strictEqual(
