...
test('Minigame State Transitions', async t => {
  ...
  await t.test('START_KABELSALAT_MINIGAME updates state correctly', () => {
    ...
    assert.strictEqual(newState.minigame.gigId, 'gig_456')
  })
  ...
  await t.test(
    ...
    () => {
      ...
      const action = {
        ...
        payload: { results: { isPoweredOn: true, timeLeft: 20 } }
      }
      const newState = gameReducer(activeState, action)
      ...
      // isPoweredOn=true, timeLeft=20 -> timeBonus = 4 -> baseReward = 50 + 40 = 90
      // tech_wizard trait -> reward = 90 * 1.5 = 135
      // stress = 0
      assert.strictEqual(newState.band.harmony, 80)
      assert.strictEqual(newState.player.money, 635)
    }
  )

  await t.test(
    'COMPLETE_KABELSALAT_MINIGAME on failure reduces harmony and sets damaged_gear modifier',
    () => {
      const activeState = {
        ...initialState,
        currentScene: GAME_PHASES.PRE_GIG_MINIGAME,
      ...
      const action = {
        type: ActionTypes.COMPLETE_KABELSALAT_MINIGAME,
        payload: { results: { isPoweredOn: false, timeLeft: 5 } }
      }
      const newState = gameReducer(activeState, action)

      assert.deepStrictEqual(newState.minigame, DEFAULT_MINIGAME_STATE)
      // isPoweredOn=false -> reward = 0, stress = 10
      // Money stays same, Harmony - 10, damaged_gear becomes true
      assert.strictEqual(newState.band.harmony, 70)
      assert.strictEqual(newState.player.money, 500)
      assert.strictEqual(newState.gigModifiers.damaged_gear, true)
    }
  )
})
