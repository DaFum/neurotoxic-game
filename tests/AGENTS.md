...
- Tests whose primary subject is repository tooling belong in `TOOLING_NODE_TESTS` (or the focused tooling Vitest config) and run through `pnpm run test:tooling`, not the production-oriented test commands.
