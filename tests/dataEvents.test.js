  ...
  ['band', 'financial', 'gig', 'transport', 'special'].forEach(cat => {
    if (EVENTS_DB[cat]) {
      ...
      EVENTS_DB[cat].forEach(event => {
        ...
        assert.ok(event.description || event.text, `Event ${event.id} missing description/text`)

        // normalize for future tests if needed, but for now just check one exists.

        assert.ok(Array.isArray(event.options) || Array.isArray(event.choices), `Event ${event.id} missing choices/options array`)
      })
    }
  })
})
