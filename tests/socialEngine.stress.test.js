...
const createRandomGameState = () => ({
  player: { 
    ...
    hqUpgrades: Math.random() > 0.5 ? ['hq_room_coffee', 'hq_room_cheap_beer_fridge'] : []
  },
  band: {
...
test('Stress Test: generatePostOptions over 10,000 random states', () => {
  for (let i = 0; i < 10000; i++) {
    ...
    assert.equal(options.length, 3, `Expected 3 options, got ${options.length} on iteration ${i}`)
...
test('Stress Test: resolvePost over 10,000 random selections and RNG rolls', () => {
  for (let i = 0; i < 10000; i++) {
    ...
    const postOption = POST_OPTIONS[Math.floor(Math.random() * POST_OPTIONS.length)]
    ...
    if (result.harmonyChange !== undefined) assert.ok(!isNaN(result.harmonyChange))
    if (result.controversyChange !== undefined) assert.ok(!isNaN(result.controversyChange))
    if (result.loyaltyChange !== undefined) assert.ok(!isNaN(result.loyaltyChange))
  }
})
...
test('Stress Test: calculateDailyUpdates over 10,000 random states', () => {
  for (let i = 0; i < 10000; i++) {
    ...
    nextState.band.members.forEach(m => {
      ...
      assert.ok(m.stamina >= 0 && m.stamina <= 100, `Stamina out of bounds for ${m.name}: ${m.stamina}`)
    })
