...
export const deterministicBootstrapConfidence = (
  values,
  seed,
  resamples = 2000
) => {
  if (!values.length)
    return {
      mean: { lower: null, upper: null, method: 'paired-bootstrap', resamples },
      median: {
        lower: null,
        upper: null,
  ...
  const interval = samples => ({
    lower: round(quantile(samples, 0.025)),
    upper: round(quantile(samples, 0.975)),
    method: 'paired-bootstrap',
    resamples
  })
...
export const pairedMetricStatistics = (
  control,
  candidate,
  { bootstrapSeed, resamples = 2000 }
) => {
  if (control.length !== candidate.length)
    throw new RangeError('Paired populations must have equal length')
  ...
  return {
    ...
    pairedDelta: {
      ...
      candidateWinRatePct: round(
        deltas.length ? (positiveCount / deltas.length) * 100 : 0
      ),
      confidence95: deterministicBootstrapConfidence(
        deltas,
        bootstrapSeed,
        resamples
      )
...
export const bankruptcyTransitions = (control, candidate) => {
  ...
  return {
    ...
    controlRatePct: round(
      control.length ? (controlCount / control.length) * 100 : 0
    ),
    candidateRatePct: round(
      candidate.length ? (candidateCount / candidate.length) * 100 : 0
    ),
    deltaRatePct: round(
      control.length
        ? ((candidateCount - controlCount) / control.length) * 100
        : 0
    ),
