...
const {
  ...
  clampPlayerMoney,
  clampBandHarmony
} = require('../src/utils/gameState/index.ts')
  ...
  for (let d = 0; d < DAYS_PER_TOUR; d++) {
    ...
    if (d % 2 === 0) {
      try {
        ...
        const net =
          financials.income.total - financials.expenses.total * spendMultiplier
    ...
    if (Math.random() < socialActivity) {
      try {
        const controversyInput = Math.max(
          0,
          Math.min(100, s.controversy + controversyBias)
        )
        ...
        s.controversy = Math.max(
          0,
          Math.min(100, s.controversy + (controversyBias > 0 ? 2 : -0.5))
        )
      } catch (err) {
        console.error(
          `[Social Simulation Error] Day ${d}:`,
          err instanceof Error ? err.message : String(err)
        )
      }
    }
