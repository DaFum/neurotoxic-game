...
- Shop surfaces that run purchases (Band HQ, `SupplyStopModal`, etc.) must guard against duplicate purchases from double-clicks by reusing `usePurchaseLock` (`runWithLock` + `processingItemId`) and forwarding `processingItemId` to `ShopItem`; do not call the raw `handleBuy` without a lock. The lock is async so a second click fired before the disabled state re-renders is dropped by the ref guard.
