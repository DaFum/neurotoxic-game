...
const DetailRow = memo(
  ({ label, value, subtext, locked, className = '' }: DetailRowProps) => (
    <div
...
const CareerOverviewSection = memo(
  ({ player, t }: { player: PlayerData } & BasicTProps) => {
...
const SocialReachSection = memo(
  ({ social, t }: { social: SocialData } & BasicTProps) => {
    const totalReach =
      (social.instagram ?? 0) +
      (social.tiktok ?? 0) +
      (social.youtube ?? 0) +
...
const VanConditionSection = memo(
  ({ player, t }: { player: PlayerData } & BasicTProps) => (
    <Panel
...
const RegionalStandingSection = memo(
  ({
    reputationByRegion,
    venueBlacklist,
    t
...
const ActiveQuestsSection = memo(
  ({ activeQuests, t }: { activeQuests: ActiveQuest[] } & BasicTProps) => {
...
const InventoryEquipmentSection = memo(
  ({ band, t }: { band: BandData } & BasicTProps) => (
    <Panel
...
const MemberTraits = memo(
  ({ member, t }: { member: BandMember } & BasicTProps) => {
    ...
    <div className='bg-void-black/60 border border-ash-gray p-4'>
      ...
      <div className='space-y-1 text-sm'>
        ...
        <DetailRow
          ...
          locked={
            !isUnlocked(
              member.baseStats?.composition ?? member.composition ?? 0
            )
          }
        />
      </div>
...
const BandMembersSection = memo(
  ({ members, t }: { members: BandMember[] } & BasicTProps) => (
    <div className='space-y-4'>
