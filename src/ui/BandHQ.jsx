...
export const BandHQ = ({ onClose }) => {
  ...
  const handleBuy = (item) => {
    ...
    if (currencyValue < item.cost) {
      addToast(`Nicht genug ${item.currency === 'fame' ? 'Fame' : 'Geld'}!`, 'error')
      return
    }
    ...
    switch (effect.type) {
      ...
      case 'unlock_hq':
        ...
        if (item.id === 'hq_coffee') {
           const newMembers = band.members.map(m => ({ ...m, mood: Math.min(100, m.mood + 20) }))
           updateBand({ members: newMembers })
           addToast('Kaffee getrunken! Mood +20', 'success')
        }
        if (item.id === 'hq_sofa') {
           const newMembers = band.members.map(m => ({ ...m, stamina: Math.min(100, m.stamina + 30) }))
           updateBand({ members: newMembers })
  ...
  const renderItem = (item) => {
    ...
    const isConsumable = item.effect.type === 'inventory_add'
    const disabled = (isOwned && !isConsumable) ||
                     (item.currency === 'fame' ? player.fame < item.cost : player.money < item.cost)

    return (
      <div key={item.id} className={`p-4 border ${isOwned && !isConsumable ? 'border-[var(--toxic-green)] bg-[var(--toxic-green)]/10' : 'border-gray-700 bg-black/60'} relative flex flex-col justify-between`}>
        <div>
          <h4 className="font-bold text-[var(--toxic-green)] mb-1">{item.name}</h4>
          <p className="text-xs text-gray-400 mb-2">{item.description}</p>
        </div>
        <div className="flex justify-between items-center mt-2">
          <span className={`font-mono text-sm ${item.currency === 'fame' ? 'text-yellow-400' : 'text-white'}`}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div
        ...
        style={{ backgroundImage: `url("${getGenImageUrl(IMG_PROMPTS.BAND_HQ_BG)}")` }}
      />
      ...
      <div className="relative w-full max-w-5xl h-[90vh] border-4 border-[var(--toxic-green)] bg-black/80 flex flex-col shadow-[0_0_50px_var(--toxic-green)]">
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-[var(--toxic-green)] scrollbar-track-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div className="bg-black/40 border border-gray-700 p-4">
                  <h3 className="text-[var(--toxic-green)] text-lg font-bold mb-4 border-b border-gray-800 pb-2">KARRIERE STATUS</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <StatBox label="Guthaben" value={`${player.money}€`} icon="€" />
                    <StatBox label="Fame" value={player.fame} icon="★" />
                    <StatBox label="Tag" value={player.day} icon="📅" />
                    <StatBox label="Follower" value={social.instagram + social.tiktok} icon="👥" />
                  </div>
                </div>

                <div className="bg-black/40 border border-gray-700 p-4">
                  <h3 className="text-[var(--toxic-green)] text-lg font-bold mb-4 border-b border-gray-800 pb-2">VAN STATUS</h3>
                  <div className="space-y-2">
                    <ProgressBar label="Tank" value={player.van.fuel} max={100} color="bg-yellow-500" />
                    <ProgressBar label="Zustand" value={player.van.condition} max={100} color="bg-blue-500" />
                    <div className="mt-2 text-xs text-gray-400">
                      Breakdown Chance: {(player.van.breakdownChance * 100).toFixed(1)}%
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Band Members */}
              <div className="bg-black/40 border border-gray-700 p-4">
                <h3 className="text-[var(--toxic-green)] text-lg font-bold mb-4 border-b border-gray-800 pb-2">BAND STATUS</h3>
                <div className="space-y-6">
                  {band.members.map((m, i) => (
                    <div key={i} className="flex items-center gap-4">
                      <div className="w-20 font-bold text-white">{m.name}</div>
                      <div className="flex-1 space-y-1">
                         <ProgressBar label="Stamina" value={m.stamina} max={100} color="bg-green-600" size="sm" />
                         <ProgressBar label="Mood" value={m.mood} max={100} color="bg-pink-600" size="sm" />
                      </div>
                    </div>
                  ))}
                </div>
                <div className="mt-6 pt-4 border-t border-gray-800">
                   <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-white font-mono">{band.inventorySlots}</span>
                   </div>
                   <div className="flex justify-between items-center">
                      <span className="text-gray-400">Harmony:</span>
                      <span className="text-[var(--toxic-green)] font-mono">{band.harmony}/100</span>
                   </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'SHOP' && (
            <div>
               <div className="mb-4 text-right font-mono text-white">GUTHABEN: <span className="text-[var(--toxic-green)]">{player.money}€</span></div>
          ...
          {activeTab === 'UPGRADES' && (
            <div>
               <div className="mb-4 text-right font-mono text-white">FAME: <span className="text-yellow-400">{player.fame}★</span></div>
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
