...
- `FrameCorners` (`src/ui/shared/FrameCorners.tsx`) is the shared four-corner frame primitive. New frame ornaments must reuse it instead of re-inlining four `UIFrameCorner` elements; pass shared corner classes via `className` and use `topLeftClassName` for an accent corner.
...
- Second exception: components that receive an already-resolved generated URL as a prop (map pins/vans, band-member sprites) render it through `FallbackImage` (`src/ui/shared/FallbackImage.tsx`), which owns the guarded `onError` swap to `getGeneratedImageFallbackUrl()`. Do not render prop-passed generated URLs with a raw `<img>` — a failed Pollinations request would leave a broken image with no fallback.
- Deliberate exception: purely decorative CSS `backgroundImage` overlays (e.g. `BandHQ`, `MerchPressModal`, `BloodBankModal`, `BrandDealsTab`, `PostGig`, `MainMenuBackground`) may call `resolveGenImageUrl(prompt, isOnline)` directly. CSS backgrounds have no `onError` path (a failed URL renders nothing — no broken-image icon, no retry loop), and the panel's border/skeleton/aspect-ratio chrome is wrong for full-bleed decoration. `resolveGenImageUrl` falls back to `navigator.onLine` when `isOnline` is omitted, so the offline SVG fallback always applies; pass a reactive `isOnline` when the surface should react to connectivity changes. Do not use raw `<img>` under this exception.
