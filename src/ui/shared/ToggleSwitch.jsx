...
const ToggleSwitchComponent = ({
  ...
  return (
    <div className={`flex items-center justify-between w-full max-w-sm border border-(--toxic-green)/30 p-3 bg-[color:var(--void-black)] ${className}`}>
      <span className="text-sm font-bold tracking-widest uppercase" aria-hidden="true">{ariaLabel}</span>
      <button
        type="button"
        onClick={handleToggle}
        className={`relative w-16 h-8 border-2 border-(--toxic-green) flex items-center p-1 transition-colors duration-75 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--toxic-green) focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--void-black)] ${isGlitching ? 'translate-x-[1px] translate-y-[1px]' : ''}`}
        aria-checked={isOn}
        role="switch"
      ...
      >
        ...
        <div className={`w-5 h-full bg-(--toxic-green) transition-transform duration-100 z-10 ${isOn ? 'translate-x-8' : 'translate-x-0'}`}>
           <div className="w-[2px] h-full bg-[color:var(--void-black)] mx-auto opacity-50"></div>
        </div>
