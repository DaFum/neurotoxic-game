...
- `calculateMerchIncome` reads `context.cityTraits?: CityTraitState` to apply city `genreBias` and `barSpendingProfile` multipliers. Threaded from `state.cityStates` via `usePostGigLogic.ts` (with `deriveCityTraits` fallback when the city key has no saved entry). Undefined `cityTraits` falls back to neutral multipliers (1.0 across the board).
