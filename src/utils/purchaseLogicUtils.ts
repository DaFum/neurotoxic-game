  ...
  switch (item.id) {
    case 'hq_room_poster_wall': {
      const clampedFame = clampPlayerFame(
        ((nextPlayerPatch.fame as number | undefined) ?? finiteNumberOr(player.fame, 0)) + 10
      )
      nextPlayerPatch.fame = clampedFame
      nextPlayerPatch.fameLevel = calculateFameLevel(clampedFame)
      messages.push({
    ...
    case 'hq_room_label': {
      ...
      nextPlayerPatch.money = clampPlayerMoney(
        ((nextPlayerPatch.money as number | undefined) ?? finiteNumberOr(player.money, 0)) +
          LABEL_CONTRACT_ADVANCE
      )
      messages.push({
    ...
    case 'hq_room_cheap_beer_fridge': {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (!member) continue
        transformedMembers.push(transform(member))
      }
