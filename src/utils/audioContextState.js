...
/**
 * Determines whether context state warrants an engine rebuild attempt.
 * @param {string} state - AudioContext state.
 * @returns {boolean} True when the current state is not recoverable by resume alone.
 */
export function shouldRebuildAudioContextState(state) {
  return state === 'closed' || state === 'unknown'
}
