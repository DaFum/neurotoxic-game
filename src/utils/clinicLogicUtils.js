...
export const calculateHealAmounts = (member, healStaminaGain, healMoodGain) => {
  const maxStamina = member.staminaMax || 100
  const currentStamina = member.stamina || 0
  const healAmountApplied = Math.min(healStaminaGain, Math.max(0, maxStamina - currentStamina))

  const currentMood = member.mood || 0
  const moodAmountApplied = Math.min(healMoodGain, Math.max(0, 100 - currentMood))
