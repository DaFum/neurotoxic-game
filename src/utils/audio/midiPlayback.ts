...
function triggerInstrumentNote(
  ...
  if (lane === 'midiDrumKit') {
    return playDrumNote(midiPitch, time, velocity, audioState.midiDrumKit || undefined)
  }
...
function getSynthConfigForLane(lane: string): { synth: Tone.PolySynth | null | undefined, dur: string, freq: string } {
  if (lane === 'bass') return { synth: audioState.bass as Tone.PolySynth, dur: '8n', freq: 'C3' }
  if (lane === 'midiBass') return { synth: audioState.midiBass as Tone.PolySynth, dur: '8n', freq: 'C3' }
  if (lane === 'midiLead') return { synth: audioState.midiLead as Tone.PolySynth, dur: '16n', freq: 'C4' }
  return { synth: audioState.guitar as Tone.PolySynth, dur: '16n', freq: 'C4' }
}
...
function isValidSongNote(n: SongLikeNote | undefined | null): n is SongLikeNote & { t: number, p: number, v: number } {
  if (!n) return false
  return isFiniteNumber(n.t) && isFiniteNumber(n.p) && isFiniteNumber(n.v)
}

function calculateNoteTime(n: { t: number }, bpm: number, tpb: number, useTempoMap: boolean, activeTempoMap: ProcessedTempoMapEntry[]): number {
...
function processSongEvents(
  ...
  const events = notes.reduce<ProcessedSongEvent[]>((acc, n) => {
    if (isValidSongNote(n)) {
      const evt = createSongEventFromNote(n, bpm, tpb, useTempoMap, activeTempoMap)
      if (evt) {
...
function sanitizeTempoEvents(rawTempo: unknown[]): { tick: number; usPerBeat: number }[] {
...
function extractTempoMap(sObj: Record<string, unknown>, tpb: number): { useTempoMap: boolean, activeTempoMap: ProcessedTempoMapEntry[] } {
  ...
  return { useTempoMap: true, activeTempoMap: preprocessTempoMap(sanitized, tpb) }
}
...
function parseSongParameters(sObj: Record<string, unknown>) {
  ...
  const notes = Array.isArray(sObj.notes) ? (sObj.notes as SongLikeNote[]) : undefined
...
function executeSongPlayback(
  ...
  delay: number,
  reqId: number,
  onEnded: ((info: MidiPlaybackEndInfo) => void) | null | undefined
) {
  Tone.getTransport().bpm.value = bpm
  audioState.part = createSongPart(res.events)

  const validDelay = isFiniteNumber(delay) ? Math.max(0, delay) : 0
  scheduleSongPlayback(res.lastTime, validDelay, reqId, onEnded as (() => void) | null)
}
...
export async function playSongFromData(
  ...
  const res = processSongEvents({ notes: p.notes }, p.bpm, p.tpb, t.useTempoMap, t.activeTempoMap)
  if (!res) return false

  executeSongPlayback(res, p.bpm, delay, prep.reqId, prep.normalizedOptions.onEnded)
  return true
}
...
function createProcessedMidiEvent(note: ParsedMidiNote, percussionTrack: boolean): ProcessedMidiEvent | null {
...
function processMidiTrackNotes(notes: ParsedMidiNote[], percussionTrack: boolean): ProcessedMidiEvent[] {
...
type SynthsContext = {
  ...
  drumSet: Tone.Sampler | null | undefined
}
...
function handleMidiEvent(time: number, value: ProcessedMidiEvent, synths: SynthsContext) {
  ...
  triggerMidiSynth(value.midiPitch, time, velocity, duration, value.frequencyNote, leadSynth, bassSynth)
}

function createTrackPart(track: ParsedMidiTrack, synths: SynthsContext): Tone.Part<unknown> | null {
...
function scheduleStopCallback(
  ...
  const finiteStopAfter = isFiniteNumber(stopAfterSeconds) ? stopAfterSeconds : null
...
function configureTransportLoop(loop: boolean, duration: number, requestedOffset: number) {
...
function getValidOffset(offset: number, duration: number): number {
  ...
  if (duration > 0 && requestedOffset >= duration) {
    logger.warn('AudioEngine', `Offset ${requestedOffset}s exceeds duration ${duration}s. Resetting to 0.`)
    requestedOffset = 0
  }
  return requestedOffset
}

function getTransportStartTime(startTimeSec: number | null, validDelay: number): number {
...
function scheduleMidiTransport(
  ...
  const validDelay = isFiniteNumber(params.delay) ? Math.max(0, params.delay) : 0
  const duration = isFiniteNumber(midi.duration) ? midi.duration : 0
  const requestedOffset = getValidOffset(params.offset, duration)

  logger.debug('AudioEngine', `Starting Transport. Delay=${validDelay}, Offset=${requestedOffset}`)

  configureTransportLoop(params.loop, midi.duration, requestedOffset)

  if (!params.loop) {
    scheduleMidiEndEvents(params.reqId, params.filename, duration, requestedOffset, params.onEnded, params.stopAfterSeconds)
  }

  const transportStartTime = getTransportStartTime(params.startTimeSec, validDelay)
...
async function prepareAudioContextForMidi(reqId: number): Promise<boolean> {
  ...
  if (!isRequestValid(reqId)) {
    logger.debug('AudioEngine', `Request cancelled during ensureAudioContext (reqId: ${reqId} vs ${audioState.playRequestId})`)
    return false
  }
  return true
}

async function loadAndValidateMidi(url: string, reqId: number, filename: string): Promise<ParsedMidi | null> {
...
async function fetchAndParseMidi(filename: string, reqId: number): Promise<ParsedMidi | null> {
...
function processMidiPlaybackParams(params: MidiPlaybackParams) {
  const defaults = { offset: 0, loop: false, delay: 0, options: {}, ownedRequestId: null }
...
export async function playMidiFileInternal(
  ...
  const reqId = initializePlaybackRequest(params.filename, params.offset, params.loop, params.ownedRequestId)
