...
- Repeat policy is enforced in `QuestLifecycle.addQuest`: `'never'` is blocked once the id is in `completedQuestIds` or a completion/reward flag is active; an unexpired `questCooldowns` entry blocks re-add for **every** repeat policy (failure-penalty `quest.cooldown` retry windows on `'never'` story quests included), not just `'cooldown'`. Gameplay code that conditionally applies side effects alongside `addQuest` (toasts, mode flags) and event conditions that surface quest offers must gate on `canAcceptQuest`/`QuestOfferEngine.canOfferQuest` instead of bespoke active-quest or `eventCooldowns` checks. `completeQuest` records `completedQuestIds`, clears `clearFlagsOnComplete`, and opens cooldowns; `checkDeadlines` applies failure penalties (controversy/harmony/loyalty), pushes `failurePenalty.flags`, clears `clearFlagsOnFail`, and records re-add cooldowns keyed by quest id. Cooldowns expire in `handleAdvanceDay`.
...
- Repeat policies have specific scope semantics: `cooldown` writes a `questCooldowns` entry keyed by `questId` (global); `perVenue` and `perRegion` write a `completedQuestScopes` entry keyed by `(questId, scopeKey)` and refuse re-add only for the same scope. `addQuest` stamps `scopeKey` from `state.currentGig?.id` or from `state.player?.currentNodeId` only when the current map node is `type: 'GIG'` (venue), or from the canonical city key derived via `getRegionKeyForLocation(state.player?.location)` (region); a scoped quest without an available scope key is refused. `player.location` itself stays the `venues:<id>.name` display key — region reputation maps, region quest events, and perRegion scopes must always go through `getRegionKeyForLocation` (`src/utils/mapUtils.ts`), never raw `player.location`. Scoped quests also **only progress** when a rule declares `match.scope` and the emitted event context matches the stamped `scopeKey`.
