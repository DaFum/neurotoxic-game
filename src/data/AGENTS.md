...
- Non-deterministic event conditions and effects (e.g. `crisis.ts` raid roll) must use `secureRandom()` from `src/utils/crypto`, not `Math.random()`. `secureRandom()` itself throws when the Crypto API is unavailable; the `Math.random()` fallback lives in `getSafeRandom()` and `getSafeUUID()` wrappers, which log once and degrade gracefully. Tests stub the imported function directly.
