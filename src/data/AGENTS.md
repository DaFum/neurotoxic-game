...
- **Gameplay anschließen**: nutze `QuestEvents.emit(state, event)` (in Reducern) oder die `applyQuestEvent`-Action (in Hooks) als einzige Andockstelle. NICHT `QuestLifecycle.advanceQuest`/`setQuestProgress` direkt aus Producer-Code aufrufen — ein Lint-Gate in `tests/node/questSystem.test.js` enforced das. Pass möglichst viel Kontext mit: `platform`, `category`, `dealType`, `brandAlignment`, `venueId`, `region` — Quests filtern darüber via `QUEST_EVENT_FILTERS` in `questProgress.ts`. Beispiel: ein Lifestyle-Post emittet `{ type: 'social_post', category: 'Lifestyle', platform: 'instagram' }`; `quest_community_outreach` (filtert auf `Lifestyle`/`Community`) zählt, `quest_drama_post` (filtert auf `Drama`) zählt nicht. Wenn der Filter fehlt, akzeptiert die Quest jeden Match — also: lieber Kontext mitschicken als später ein neuen Filter eintragen.
...
- Repeat policies have specific scope semantics: `cooldown` writes a `questCooldowns` entry keyed by `questId` (global); `perVenue` and `perRegion` write a `completedQuestScopes` entry keyed by `(questId, scopeKey)` and refuse re-add only for the same scope. `addQuest` stamps `scopeKey` from `state.currentGig?.id ?? state.player?.currentNodeId` (venue) or `state.player?.location` (region); a `perVenue`/`perRegion` quest without an available scope key is refused. Scoped quests also **only progress** when the emitted event's `venueId`/`region` matches the stamped `scopeKey` — `QuestProgress.applyEvent` enforces this in addition to the existing repeat-policy gating in `addQuest`, so e.g. `quest_local_legend` only counts fame earned in its stamped region. `fame_gained` emits with `region: player.location` to make that match possible.
