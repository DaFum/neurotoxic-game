...
- **Gameplay anschließen**: nutze Producer in `src/quests/producers/*` plus `QuestEvents.emit(state, event)` (in Reducern) oder die `applyQuestEvent`-Action (in Hooks). NICHT `QuestLifecycle.advanceQuest`/`setQuestProgress` direkt aus Producer-Code aufrufen — ein Gate in `tests/node/questSystem.test.js` enforced das. Producer sollen Kontext mitschicken (`platform`, `postCategory`, `dealType`, `brandAlignment`, `venueId`, `region`, `assetKind`, `itemId`, `minigameId`, `tags`); Quests filtern ausschließlich über `progressRules.match`.
