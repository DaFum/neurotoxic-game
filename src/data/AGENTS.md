...
- Repeat policies have specific scope semantics: `cooldown` writes a `questCooldowns` entry keyed by `questId` (global); `perVenue` and `perRegion` write a `completedQuestScopes` entry keyed by `(questId, scopeKey)` and refuse re-add only for the same scope. `addQuest` stamps `scopeKey` from `state.currentGig?.id` or from `state.player?.currentNodeId` only when the current map node is `type: 'GIG'` (venue), or from `state.player?.location` (region); a scoped quest without an available scope key is refused. Scoped quests also **only progress** when a rule declares `match.scope` and the emitted event context matches the stamped `scopeKey`.
