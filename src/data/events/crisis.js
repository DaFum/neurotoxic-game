...
/**
 * Erstellt ein Composite-Effektobjekt, das gegebene Effekte zusammenfasst und einen Cooldown für ein Ereignis anhängt.
 *
 * @param {string} eventId - Kennung des Ereignisses, das im angehängten Cooldown verwendet wird.
 * @param {Array<object>} effects - Array von Effektobjekten, die in das Composite übernommen werden.
 * @param {string} [description] - Optionale Beschreibung, die dem Composite als `description`-Feld hinzugefügt wird.
 * @returns {object} Das Composite-Objekt mit `type: 'composite'` und einer `effects`-Liste, die die übergebenen Effekte sowie ein `{ type: 'cooldown', eventId }`-Element enthält.
 */
