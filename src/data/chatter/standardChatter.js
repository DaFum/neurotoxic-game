export const CHATTER_DB = [
  ...
  { text: 'Did we pack the spare snare?', weight: 1, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'I should have been a dentist.', weight: 0.2, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'Did we forget the banner again?', weight: 1, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'Imagine explaining this life to a normal person.', weight: 0.2, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'Next person who says “scenic route” is carrying the kick drum.', weight: 0.8, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  { text: 'My neck is permanently shaped like this headrest.', weight: 0.6, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  { text: 'We’re one pothole away from turning into a percussion ensemble.', weight: 1, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'If we stop one more time, we’ll arrive next week.', weight: 0.7, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'We should sell “van air” in jars. Limited edition.', weight: 0.5, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'Are we touring or just driving in circles to avoid our feelings?', weight: 0.4, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'If the van breaks, we’re carrying amps like pilgrims.', weight: 0.7, category: 'travel', speaker: 'Lars' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'We should write a song called “Rest Stop Romance”.', weight: 0.4, category: 'travel', speaker: 'Marius' , condition: state => state.currentScene === 'TRAVEL_MINIGAME' || state.currentScene === 'OVERWORLD' },
  ...
  { text: 'I’m not nervous. My stomach is just… touring.', weight: 1, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Marius' },
  { text: 'If we start late, I’m blaming the stairs. Again.', weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  ...
  { text: 'If the click track fails, we become jazz. Against our will.', weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  ...
  { text: 'I can’t tell if I’m exhausted or enlightened.', weight: 2, condition: state => state.currentScene === 'POSTGIG', speaker: 'Marius' },
  ...
  { text: 'That encore felt like a fistfight with gravity.', weight: 3, condition: state => state.currentScene === 'POSTGIG', speaker: 'Lars' },
  ...
  { text: 'My ears are ringing in a key that doesn’t exist.', weight: 2, condition: state => state.currentScene === 'POSTGIG', speaker: 'Marius' },
  ...
  { text: 'I’m too tired to be annoyed, but here we are.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 30) },
  { text: 'If the van makes that noise again, I’m naming it and hating it.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 25) },
  { text: 'I want to be a person again, not a touring object.', weight: 10, condition: state => state.band.memberstate.some(m => m.mood < 20) },
  { text: 'Everything is sticky and I’m emotionally sticky too.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 25) },
  { text: 'I’m not “quiet”, I’m conserving rage.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 30), speaker: 'Lars' },
  { text: 'If we don’t get food soon, I’m eating a cymbal.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 25), speaker: 'Lars' },
  { text: 'I’m allergic to optimism right now.', weight: 8, condition: state => state.band.memberstate.some(m => m.mood < 25), speaker: 'Marius' },
  ...
  { text: 'That groove felt illegal. Let’s do it again tomorrow.', weight: 5, condition: state => state.band.memberstate.some(m => m.mood > 85) },
  { text: 'I’m vibrating with serotonin and tinnitus.', weight: 4, condition: state => state.band.memberstate.some(m => m.mood > 80), speaker: 'Marius' },
  { text: 'We’re actually getting good at this. Wild.', weight: 5, condition: state => state.band.memberstate.some(m => m.mood > 85) },
  { text: 'Tonight’s gonna be one of those “remember this” nights.', weight: 5, condition: state => state.band.memberstate.some(m => m.mood > 90) },
  { text: 'If we keep playing like this, we’ll scare ourselves.', weight: 4, condition: state => state.band.memberstate.some(m => m.mood > 80), speaker: 'Lars' },
  { text: 'I feel unstoppable. Somebody stop me from buying new gear.', weight: 3, condition: state => state.band.memberstate.some(m => m.mood > 90) },
  ...
  { text: 'If the venue offers free drinks, that’s basically income.', weight: 8, condition: state => state.player.money < 150, speaker: 'Marius' },
  ...
  { text: 'If we hit 1k, I’m demanding a tour bus. A tiny one.', weight: 3, condition: state => (state.social?.instagram || 0) > 1000, speaker: 'Lars' },
  { text: 'We got recognized at the merch table. I panicked.', weight: 3, condition: state => (state.social?.instagram || 0) > 900, speaker: 'Matze' },
  { text: 'Viral means people will actually notice mistakes. Great.', weight: 5, condition: state => (state.social?.viral || 0) > 0, speaker: 'Marius' },
  ...
  { text: 'Stendal feels like the tutorial level. Comforting and cursed.', weight: 8, condition: state => state.player.location === 'Stendal', speaker: 'Marius' },
 ...
 { text: 'HARDER!', weight: 5, condition: state => state.currentScene === 'GIG' && state.band.memberstate.some(m => m.stamina > 80), speaker: 'Lars' },
  { text: 'DON’T DROP IT!', weight: 5, condition: state => state.currentScene === 'GIG' && state.band.memberstate.some(m => m.stamina > 75) },
  { text: 'My forearms are filing for divorce!', weight: 5, condition: state => state.currentScene === 'GIG' && state.band.memberstate.some(m => m.stamina < 30), speaker: 'Matze' },
  { text: 'If I miss again, just stare at me dramatically.', weight: 4, condition: state => state.currentScene === 'GIG' && state.band.memberstate.some(m => m.stamina < 35), speaker: 'Marius' },
  { text: 'KEEP IT TOGETHER!', weight: 5, condition: state => state.currentScene === 'GIG' && state.band.memberstate.some(m => m.stamina > 80) },
  ...
  { text: 'Someone say something before I start a fight with the GPS.', weight: 8, condition: state => state.band.harmony < 30, speaker: 'Lars' },
  ...
  { text: 'Nobody\'s annoyed. That\'s basically a miracle on tour.', weight: 4, condition: state => state.band.harmony > 85, speaker: 'Marius' },
  { text: 'Even the van sounds better when we get along.', weight: 4, condition: state => state.band.harmony > 90 },
  { text: 'Harmony this high should be illegal on a Tuesday.', weight: 3, condition: state => state.band.harmony > 90, speaker: 'Lars' },
  ...
  { text: 'Fuel gauge says "pray". Got it.', weight: 10, condition: state => state.player.van.fuel < 15, speaker: 'Lars' },
  { text: 'We should start pushing. For practice.', weight: 8, condition: state => state.player.van.fuel < 20, speaker: 'Marius' },
  ...
  { text: 'Van sounds healthy. Suspicious, but healthy.', weight: 3, condition: state => state.player.van.condition > 90, speaker: 'Lars' },
  ...
  { text: 'First few shows. Let\'s not embarrass ourselves.', weight: 8, condition: state => state.player.day <= 3, speaker: 'Marius' },
  ...
  { text: 'I\'ve lost track of what day it is. That\'s a good sign. Or terrible.', weight: 5, condition: state => state.player.day >= 15, speaker: 'Marius' },
  { text: 'We\'ve been out long enough that home feels fictional.', weight: 8, condition: state => state.player.day >= 25 },
  { text: 'Day twenty-something. My body is a ruin. My spirit is a weapon.', weight: 8, condition: state => state.player.day >= 25, speaker: 'Lars' },
  ...
  { text: 'Veteran roads. We know every pothole by name.', weight: 4, condition: state => state.player.totalTravels >= 20, speaker: 'Lars' },
  ...
  { text: 'One day they\'ll know our name. Today is not that day.', weight: 6, condition: state => (state.player.fame || 0) < 50, speaker: 'Marius' },
  ...
  { text: 'We\'re not underground anymore. We\'re ground level?', weight: 4, condition: state => (state.player.fameLevel || 0) >= 3, speaker: 'Marius' },
  ...
  { text: 'We\'re headlining now. Headlining. Let that sink in.', weight: 2, condition: state => (state.player.fameLevel || 0) >= 5, speaker: 'Lars' },
  ...
  { text: 'Lars is going to lose it when he finds out we have no drum parts.', weight: 10, condition: state => state.band.inventory.drum_parts === false },
  ...
  { text: 'If a head breaks, we\'re done. No drum parts. Zero.', weight: 8, condition: state => state.band.inventory.drum_parts === false, speaker: 'Lars' },
  ...
  { text: 'Hot meal before the set. I could cry.', weight: 5, condition: state => state.gigModifiers?.catering === true, speaker: 'Lars' },
  { text: 'We actually did promo for this one. Like professionals.', weight: 5, condition: state => state.gigModifiers?.promo === true },
  { text: 'Posters are up, socials are posted. We\'re trying.', weight: 5, condition: state => state.gigModifiers?.promo === true, speaker: 'Marius' },
  { text: 'Soundcheck secured. No more guessing the monitor mix.', weight: 5, condition: state => state.gigModifiers?.soundcheck === true },
  { text: 'A real soundcheck? We\'re basically royalty.', weight: 5, condition: state => state.gigModifiers?.soundcheck === true, speaker: 'Lars' },
  ...
  { text: 'Lucky streak. Don\'t jinx it. DON\'T JINX IT.', weight: 5, condition: state => (state.band.luck || 0) > 3, speaker: 'Marius' },
  { text: 'The universe is on our side today. Enjoy it while it lasts.', weight: 4, condition: state => (state.band.luck || 0) > 4 },
  { text: 'Green lights, free parking, no flat tires. What timeline is this?', weight: 4, condition: state => (state.band.luck || 0) > 5, speaker: 'Lars' },
  ...
  { text: 'We\'re cursed. That\'s the only explanation.', weight: 10, condition: state => (state.band.luck || 0) < -4, speaker: 'Marius' },
  { text: 'At this point even the van is giving us side-eye.', weight: 8, condition: state => (state.band.luck || 0) < -3, speaker: 'Lars' },

  // --- MINIGAME: TRAVEL ---
  { text: 'Eyes on the road!', weight: 5, condition: state => state.currentScene === 'TRAVEL_MINIGAME', speaker: 'Lars' },
  ...
  { text: 'Is that a shortcut?', weight: 3, condition: state => state.currentScene === 'TRAVEL_MINIGAME', speaker: 'Marius' },
  ...
  { text: 'Hurry up, doors open soon!', weight: 5, condition: state => state.currentScene === 'PRE_GIG_MINIGAME', speaker: 'Lars' },
  { text: 'Careful with the merchandise box.', weight: 4, condition: state => state.currentScene === 'PRE_GIG_MINIGAME' },
  { text: 'Why is this case so heavy?', weight: 3, condition: state => state.currentScene === 'PRE_GIG_MINIGAME', speaker: 'Marius' }
