export const CHATTER_DB = [
  ...
  // --- PRE-GIG (Preparation) ---
  { text: 'Where is the sound guy?', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'I need a beer before we start.', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  ...
  { text: "Let's stick to the setlist this time, okay?", weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  ...
  { text: 'No new songs tonight. Last time was a trainwreck.', weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  ...
  { text: 'If my in-ears cut out, I’m playing by memory and spite.', weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  { text: 'One more “can you turn up” and I’m turning up the whole band.', weight: 2, condition: state => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(state.currentScene), speaker: 'Lars' },
  { text: 'Which one of you touched my amp settings?', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'If the stage is tiny, we play like it’s a stadium.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'Can we get water on stage or is it “suffer artistically” tonight?', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'Why is the power strip always missing when we need it most?', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'I’m not nervous. My stomach is just… touring.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene), speaker: 'Marius' },
  { text: 'If we start late, I’m blaming the stairs. Again.', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene), speaker: 'Lars' },
  { text: 'Does anyone remember the count-in for the new intro? Anyone?', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'If the lights blind me, I’m playing by smell.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene), speaker: 'Matze' },
  { text: 'I need my monitor to be louder than my thoughts.', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'We should label our cables. We won’t, but we should.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'If the click track fails, we become jazz. Against our will.', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene), speaker: 'Lars' },
  { text: 'Tell me again why we didn’t bring a spare guitar?', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene), speaker: 'Matze' },
  { text: 'I can’t find the tuner. This is how wars start.', weight: 2, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'If the stage is wobbling, we’re calling it “energy”.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
  { text: 'Who’s doing the intro speech? Please say “not me”.', weight: 1, condition: s => ['PREGIG', 'PRE_GIG_MINIGAME'].includes(s.currentScene) },
 ...
 { text: 'HARDER!', weight: 5, condition: s => s.currentScene === 'GIG' && s.band.members.some(m => m.stamina > 80), speaker: 'Lars' },
  ...
  // --- MINIGAME: TRAVEL ---
  { text: 'Eyes on the road!', weight: 5, condition: s => s.currentScene === 'TRAVEL_MINIGAME', speaker: 'Lars' },
  { text: 'Look out for that pothole!', weight: 5, condition: s => s.currentScene === 'TRAVEL_MINIGAME' },
  { text: 'We’re making good time.', weight: 3, condition: s => s.currentScene === 'TRAVEL_MINIGAME' },
  { text: 'Is that a shortcut?', weight: 3, condition: s => s.currentScene === 'TRAVEL_MINIGAME', speaker: 'Marius' },
  { text: 'Hold on tight!', weight: 4, condition: s => s.currentScene === 'TRAVEL_MINIGAME' },

  // --- MINIGAME: ROADIE ---
  { text: 'Don’t drop the amp!', weight: 5, condition: s => s.currentScene === 'PRE_GIG_MINIGAME' },
  { text: 'My back!', weight: 3, condition: s => s.currentScene === 'PRE_GIG_MINIGAME', speaker: 'Matze' },
  { text: 'Hurry up, doors open soon!', weight: 5, condition: s => s.currentScene === 'PRE_GIG_MINIGAME', speaker: 'Lars' },
  { text: 'Careful with the merchandise box.', weight: 4, condition: s => s.currentScene === 'PRE_GIG_MINIGAME' },
  { text: 'Why is this case so heavy?', weight: 3, condition: s => s.currentScene === 'PRE_GIG_MINIGAME', speaker: 'Marius' }
