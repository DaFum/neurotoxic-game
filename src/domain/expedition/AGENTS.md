...
- A seeded gate proves an event *could* occur at a route step, never that it did. `sourceId`, `resolvedEventSourceIds` and ledger entries are all save-authored and cannot authenticate a reward. Unmaterialized `event_rare` entries are dropped on load for that reason; do not re-admit them without reducer-authored evidence a save cannot construct.
