...
- `handleAssetRiskEventTriggered`: subtracts the dynamic `payload.conditionLoss` from `asset.condition` (clamped to `[0, 100]`). It is a direct action path, not the automatic daily risk-event path; daily risk events are applied by `rollAssetRiskEvents` inside `handleAdvanceDay`.
