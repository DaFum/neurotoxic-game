...
export class AmpWaveManager {
  ...
  // Impact: Reduces frame render time and eliminates DOM style resolution overhead during amp minigame gameplay.
  colors: {
    electricBlue: number
    toxicGreen: number
    bloodRed: number
   ...
   *
   * @param container - The PIXI Container where the wave graphics will be rendered.
   * @param app - The PIXI Application instance providing screen dimensions.
   */
  ...
  /**
   * Renders the target and current sine waves based on state parameters.
   *
   * @remarks
   * This method clears the existing graphics and redraws the waves every frame.
   * Visual properties like amplitude, jitter, color, and width are dynamically adjusted
   * based on active gameplay modifiers such as overheat or overdrive.
   *
   * @param targetFreq - The required frequency for perfect calibration.
   * @param currentFreq - The current frequency generated by user input.
   * @param time - The elapsed time used to offset the wave phase.
   * @param isOverdriveActive - Indicates if the overdrive power-up is active.
   * @param isOverheat - Indicates if the amp is currently overheating.
   * @param isAnomalyActive - Indicates if a distortion anomaly is altering rendering behavior.
   * @param interference - The amount of randomized jitter applied to the target wave.
   * @param isHijackActive - Indicates if a hostile event has hijacked the amp visuals.
   */
  drawWaves(
    targetFreq: number,
    currentFreq: number,
  ...
  /**
   * Cleans up rendering resources and removes graphics from the stage.
   */
  dispose() {
