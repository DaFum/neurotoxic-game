...
- `gigGapDays` sets how *often* a scenario plays; `gigCadencePolicy` sets *which* days. `SHIPPED_GIG_CADENCE_POLICY` lives in the simulation harness, not runtime code under `src/`: the shipped `first-income` model plays the first reachable paid gig, then applies the configured gap. `SCENARIOS` must not pin `gigCadencePolicy`. Full simulation and experiment reports use `SIMULATION_CONSTANTS.seedNamespace` with 2,000 runs per main scenario; changing the namespace creates new unpaired cohorts, so published old/new comparisons are descriptive rather than paired effect estimates.
...
- A running minimum has to be sampled after every money-moving CALL in the day loop, not once per day and not once per group of calls (a negative event followed by a brand-deal advance would hide the trough between them). Mutations inside a single helper are deliberately not sampled individually — that would mean threading an observer through the production-mirroring helpers. `lowestMoneyBeforeFirstGig` sampled only after the daily tick, leaving events, maintenance, refuelling, the shop, asset investment and the travel cost outside the window — so the reported minimum could sit ABOVE the balance at the end of that same window. A relief lever is the same trap in reverse: the daily tick and the emergency grant are two money moves in one block, and sampling only after the grant reported the lifted balance as the trough (€40 + a €250 grant reads as €290) — exactly the number a grant candidate is diagnosed on, and invisible while the shipped no-op has `emergencyGrant: 0`. `observeEarlyRunwayMoney()` is called at each of those points; `tests/node/game-balance-simulation.test.js` pins `lowestMoneyBeforeFirstGig <= moneyBeforeFirstGig` and, under a day-1-capped grant, `lowestMoneyBeforeFirstGig <= emergencyGrantTriggerMoney`.
- Every generated balance artifact carries `sourceFingerprint`, `generatorFingerprint`, `seedNamespace`, `runsPerScenario`, `workingTreeDirty`, and `artifactSchemaVersion`. Provenance validation recomputes the two SHA-256 fingerprints from the current relevant sources and generator; it does not require a clean tree, a source commit, or a reports-only commit. `workingTreeDirty` remains diagnostic, while matching fingerprints are authoritative.
...
- `BALANCE_SOURCE_FILES` in `scripts/utils/balance-report-metadata.mjs` must list every module that can move a number in the generated reports — economy scalars, fame rewards, catalogue prices, event data and `crypto.ts` (RNG batching), not just the harness. A file that changes results without changing `sourceFingerprint` makes the published reproducibility claim false; `tests/node/balanceSourceFiles.test.js` pins the required entries.
...
- Balance diagnostics must distinguish runtime event trigger opportunities from events actually resolved by the simulator. Quest registry inventory is not quest lifecycle coverage; when offers, progress, completion, expiry, and rewards are not executed, report `insufficient_evidence` instead of interpreting the partial event counters as total world density. Frequency warnings use the ten-hop tour or reached trigger opportunities as their denominator.
