...
- Audio: `AudioManager` consumes playback/mute/transport state strictly through `audioEngine` facade wrappers (no direct Tone.js access); rhythm loop/input timing now read `audioEngine.getGigTimeMs()` + `audioEngine.getTransportState()` as the single runtime clock source, while ambient playback still starts from main-menu tour actions via `AudioManager.startAmbient()` with OGG-first and MIDI fallback behavior.
