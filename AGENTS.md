...
- Release gating in `scripts/game-balance-experiments.mjs` has TWO blocking gates, and both must be evaluated *inside* the candidate search, per combination: the paired calibration validation and `measureHoldoutGate` (hard `KPI_TARGETS.bankruptcyMax` caps on the `#holdout` stream). Selecting on calibration alone and applying the holdout afterwards is what let the neutral no-op ship as `accepted-for-production-partial` while 125 of 126 pairs were never asked whether they would hold. Screening aborts at the first breached cap (cheapest rejection, `holdoutGateScenarios()` orders by ascending cap); the combination the report is written against must be re-measured with `abortOnBreach: false`, or the artifact publishes untouched caps as "unmeasured" and that reads as config drift.
