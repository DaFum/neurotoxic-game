...
- Currency strings emitted into toast `options` are baked at dispatch time. Pass `formatCurrency(value, undefined, signDisplay)` from reducers and action creators (`clinicReducer.ts`, `socialReducer.ts`, `purchaseLogicUtils.ts`, `questLifecycle.ts`) and consume the formatted string with a bare `{{amount}}` placeholder in the locale templates; remove any hardcoded `€` glyphs from `public/locales/{en,de}/ui.json`. When formatting in a component, use `i18n.language` from `useTranslation()` instead.
