...
- Audio: The audio architecture has been updated with the following principles:
  ...
  - Gig note parsing and playback now resolve excerpt windows via `resolveSongPlaybackWindow` (`excerptEndMs - excerptStartMs` → `excerptDurationMs` → `durationMs`) so gameplay timing matches audio excerpt playback.
