...
- A symbol is only a true orphan when it has none of `usedBy`, `usedByTests`, `referencedByLocal`, or `referencedInFile`. `usedBy` covers static imports and resolved dynamic `import()` calls (the latter carry `dynamic: true`, so lazily route-split scenes are not false positives). `referencedByLocal` lists same-file exported referrers (calls, JSX, dispatch-table membership, bare reads); `referencedInFile` is a boolean that also captures references from module-private helpers. Do not treat an empty `usedBy` alone as dead code.
