...
**Status:** every finding below carries a per-finding **Status** line. The snippets are intentionally left as the original findings so the document stays a faithful before/after record.
**Legende:** ✅ Bearbeitet = im PR behoben (Fix-Commits `5df1e8a`, `f16ac7a`, `3b3c90e`) · ⏭️ Zurückgestellt/Kein Eingriff = bewusst nicht geändert (mit Begründung) · ~~durchgestrichen~~ = Befund zurückgezogen.
...
- **Status:** ✅ Bearbeitet — `translationUtils.ts` importiert `isForbiddenKey` jetzt aus `./objectUtils`.
...
- **Action: FIX** — use `?? 0`.
- **Status:** ✅ Bearbeitet — beide Stellen (119, 439) auf `?? 0` umgestellt.
...
- **Status:** ✅ Bearbeitet (teilweise) — `questAdd.ts:77` → `?? []`; `crypto.ts` → `?? ''`. ⏭️ *Bewusst nicht geändert:* `rhythmUtils.ts:32,35` behält `||`, da `0` dort ein **ungültiger** bpm/difficulty-Wert ist — `??` würde `0` fälschlich durchlassen (das `||` ist hier das korrektere Verhalten).
...
- **Status:** ✅ Bearbeitet — `@deprecated`-JSDoc auf der numerischen Überladung von `checkViralEvent` ergänzt.
...
- **Status:** ⏭️ *Zurückgestellt (Deferred)* — bewusst nicht geändert: `getCityKeyFromVenueId` liefert vertraglich `string ('')` an mehrere Aufrufer; eine Umstellung auf `null` ist ein breiter, riskanter Eingriff für einen LOW-Befund.
...
- `src/scenes/kabelsalat/useKabelsalatState.ts:24` — `faultReason` initialized as `''` (string) but consumer props (`KabelsalatBoard`/overlays) declare `string | null`, forcing defensive `??`. **Action: FIX** — pick one type end-to-end.
  - **Status:** ⏭️ *Zurückgestellt (Deferred)* — `string` → `string | null` ist bereits typsicher (kein Fehler); ein Verengen der Consumer-Props ist eine Mehrdateien-Änderung ohne Verhaltensnutzen.
- `src/scenes/kabelsalat/hooks/useKabelsalatTimer.ts:11-12` — a `import type { SocketId }` sits below code that already uses a sibling import; compiles fine, but breaks conventional import ordering. **Action: FIX** (style only).
  - **Status:** ✅ Bearbeitet — Type-Import an den Anfang vor `SOCKET_COUNT` verschoben.
...
- **Status:** ✅ Bearbeitet — beide Stellen (`assetTicks.ts:166-167`, `systemReducer.ts:487`) mit `finiteNumberOr` umschlossen.
...
- **Status:** ✅ Bearbeitet — gemeinsame Helfer `buildSongDensityEvents`/`accumulateDensityCounts` extrahiert; beide Builder nutzen sie.
...
- **Status:** ✅ Bearbeitet — `findActiveQuestIndex` extrahiert; beide Aufrufer nutzen ihn.
...
- **Status:** ✅ Bearbeitet — `isRivalInPlayerLocation`-Helfer extrahiert; beide Stellen nutzen ihn.
...
- **Status:** ✅ Bearbeitet — gemeinsames `selectTop3ByScore` in neuem `src/utils/topSelection.ts`; beide Aufrufer umgestellt.
...
- **Status:** ⏭️ *Kein Eingriff nötig* — `migratePlayerLocation` ruft bereits `normalizeVenueId` auf; die verbleibende `.slice(0,-5)`-Vorverarbeitung ist ein bewusster, eng begrenzter Legacy-Guard. Nicht geändert (Verhaltensänderungsrisiko > LOW-Nutzen).
...
- **Status:** ✅ Bearbeitet — Duplizierung via gemeinsame Helfer beseitigt (§2.1); `buildSongChartDensity` bleibt als getestete öffentliche Util erhalten.
...
- **Status:** ⏭️ *Zurückgestellt / Kein Eingriff* — der Export wird von `tests/node/errorHandler.test.js` importiert und ist damit eine getestete öffentliche API, nicht tot. Nicht gelöscht.
...
- **Status:** ✅ Bearbeitet — `quest_prove_yourself`/`quest_apology_tour`/`quest_ego_management` referenzieren jetzt `QUEST_BACK_FROM_PIT`/`QUEST_SINCERE_REDEMPTION`/`QUEST_BAND_PACT`.
...
- `src/utils/errors/index.ts:3` — `ErrorCategory` re-export has no `src/` consumer (`ErrorSeverity` does). **Action: DELETE** the re-export or document as intended API.
  - **Status:** ⏭️ *Kein Eingriff* — der Barrel-Re-Export ist an Tests gekoppelt; Nutzen des Entfernens ≈ null. Belassen.
- `src/utils/errors/types.ts` — `GameError.State` and `GameError.Audio` static factories are never called (callers use `new GameError(...)`/`StateError`). **Action: DELETE.**
  - **Status:** ✅ Bearbeitet — beide statischen Factories entfernt (keine Nutzung in `src/` oder `tests/`).
...
- **Status:** ✅ Bearbeitet (teilweise) — hartkodierter Betrag `-$250` aus `crisis.opt1` (EN+DE) entfernt (Konventionsverstoß behoben). Der unerreichbare Default-Zweig bleibt unangetastet (kein Verhaltensänderungsrisiko).
...
- **Status:** ✅ Bearbeitet — losgelöster JSDoc-Block entfernt.
...
- **Status:** ✅ Bearbeitet — äußeres `Math.max` entfernt.
...
- **Status:** ✅ Bearbeitet — verschachtelter Guard zu `else if (hasName && !id.includes(':'))` zusammengefasst (verhaltensgleich).
...
- **LOW** — `src/utils/purchaseLogicUtils.ts:~640-667` (`applyUnlockHQ`): the HQ bundle includes `hq_room_cheap_beer_fridge` but the inner switch has no explicit case, so it falls to `default` (mood +5), possibly diverging from that item's daily-tick mood effect. **Action: FIX** — add an explicit case or confirm intended.
  - **Status:** ✅ Bearbeitet — expliziter `case 'hq_room_cheap_beer_fridge'` (Mood +5, verhaltenserhaltend) mit Kommentar ergänzt; kein stiller Fall-through mehr.
- **LOW** — `src/utils/logger.ts:61,63,95,97` reads/writes `localStorage` directly instead of via the `storage.ts` safe helpers. Likely justified (logger is a low-level bootstrap dep). **Action: INTEGRATE** if no circular-dep blocker, else document.
  - **Status:** ⏭️ *Kein Eingriff (Non-finding)* — bewusst belassen: der Logger ist eine Low-Level-Bootstrap-Abhängigkeit; das Routing über `storage.ts` würde einen Zirkelbezug riskieren.
