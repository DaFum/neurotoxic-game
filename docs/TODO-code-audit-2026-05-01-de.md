...
- [ ] **Tuning‑Konstanten in eine Balance‑Konfiguration auslagern**: `economyEngine` enthält viele eingekapselte Konstanten; verschiebe sie in ein versioniertes Tuning‑Objekt für einfacheres A/B‑Balancing.
  ...
  - **Status:** ERLEDIGT (2026-08-02, Issue #2658) — [`src/config/balance.ts`](../src/config/balance.ts) ist jetzt die zentrale Quelle für die Gruppen `attendance` / `penalties` / `modifiers` / `caps`, trägt `configVersion` und validiert sich beim Modul‑Init über `parseBalanceConfig`; eine ungültige Config wirft damit beim Boot einen deskriptiven Fehler. `src/utils/economy/constants.ts` leitet seine Exporte daraus ab, und `calculateGigModifierCost` bekommt die Config als Parameter (Default: Produktions‑Config) statt sie im Modul‑Scope zu destrukturieren. Beide Dateien sind in `BALANCE_SOURCE_FILES` gepinnt, damit eine Lever‑Änderung den Report‑Hash nicht unverändert lässt. Abgedeckt durch `tests/node/balanceConfig.test.js`.
  - _Offen_: `EXPENSE_CONSTANTS` (Tages‑/Essens‑/Übernachtungs‑/Equipment‑/Admin‑Kosten) und die Merch‑Profile liegen weiterhin neben der Engine, und andere Engine‑Funktionen als `calculateGigModifierCost` lesen die abgeleiteten Konstanten weiter im Modul‑Scope. Schrittweise migrieren, sobald der jeweilige Bereich ohnehin angefasst wird.
...
- [x] **`postMessage` / `BroadcastChannel` origin Validation auditieren**.
  - **Status:** BESTANDEN (2026-08-02, Issue #2655) — In `src/` existiert **kein** `postMessage`-Sender, kein `message`-Listener, kein `BroadcastChannel`, kein `MessageChannel`, kein `Worker`/`SharedWorker` und kein `navigator.serviceWorker`-Message-Handling. Damit entfallen alle drei Prüfdimensionen: kein Ziel-Origin (keine Sender), keine `event.origin`/`event.source`-Prüfung nötig (keine Listener), kein Payload-Guard nötig (keine Payloads). Die vollständige Liste der `addEventListener`-Ziele in `src/` lautet `keydown`, `keyup`, `resize`, `visibilitychange`, `online`, `offline`, `beforeunload`, `unhandledrejection` — keines transportiert Cross-Origin-Daten. Kein iframe, kein `window.opener`. Tab-übergreifende Koordination läuft über `localStorage`-Keys (`unlockManager`-Marker), nicht über Messaging. Einzige angrenzende Fläche ist das Service-Worker-Update-Messaging, das vollständig in der Third-Party-Abhängigkeit `vite-plugin-pwa` / `workbox-window` liegt: [`src/components/ReloadPrompt.tsx`](../src/components/ReloadPrompt.tsx) ruft nur `useRegisterSW` und `updateServiceWorker` aus dem Modul [`virtual:pwa-register/react`](../src/types/ambient.d.ts) auf, und [`vite.config.js`](../vite.config.js) nutzt die Standard-Strategie `generateSW` mit `clientsClaim: false` und `skipWaiting: false` ohne eigenen Service Worker oder Message-Handler. **Kein unvalidierter Origin, keine Injection-Fläche, kein Folge-Defect-Issue nötig.** Bei Einführung von iframe, Worker oder eigenem Service-Worker-Kanal erneut prüfen.
- [x] **StorageAdapter Fallback für Private Browsing testen**: `localStorage.setItem` kann eine `DOMException` werfen — Adapter muss catchen und in‑Memory fallbacken.
  - **Status:** (2026-08-02, Issue #2654) — Übergangs-Guard in `src/utils/storage.ts` (`writeStorageItem` / `readStorageItem` / `removeStorageItem` + `isStorageDegraded`), abgedeckt durch `tests/context/storageFallback.test.tsx`. Die `StorageAdapter`-Abstraktion (§8.10.1) übernimmt das später als `InMemoryAdapter`.
