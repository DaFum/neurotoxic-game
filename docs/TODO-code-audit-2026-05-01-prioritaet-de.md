...
Bereits erledigte Punkte (§2 reiner Resolver, §3 Idempotenz und Routing‑Contract) sind hier nicht mehr gelistet. §1 Invarianten‑Tests und §3 Abbruchrisiko sind nur teilweise erledigt; ihre Restarbeit ist unten als 1.3rest bzw. 3.3rest aufgeführt.
...
| 8.5.1   | Migrationskette formalisieren                                  | M       | `version` ist bereits persistiert (`usePersistence.ts`) und `handleLoadGame` führt eine inline `< 2`‑Migration aus. Die Lücke ist die Kette aus reinen, sequenziell anwendbaren Migrationen — nicht eine fehlende Version. |
| 8.5.6   | Migrationsfehler nicht‑destruktiv abfangen                     | S       | Gehört zwingend zu 8.5.1. Der rohe Save muss vor dem Fallback auf `createInitialState()` unter einem Quarantäne‑Key erhalten bleiben, sonst tauscht der Fix einen Boot‑Absturz gegen stillen Datenverlust.                 |
...
| 3.3rest | `tourSuccess` an `calcCancellationRisk` durchreichen           | S       | Die UI zeigt heute das Basisrisiko, die Engine würfelt `chance * (1 - tourSuccess)`. Das Badge überschätzt das Risiko, sobald `tourSuccess > 0` — eine falsche Zahl vor einer Kaufentscheidung.                            |
...
| ------ | --------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------- |
| 5.1    | Stabiler `runSeed` statt `Date.now()`                     | M       | 8.6.4 Map‑Fuzzing, 7.13 Seed‑Sharing, 7.9 Weekly Seeds, reproduzierbare Bug‑Reports |
| 6.2    | Fuzz‑Harness für feindliche Payloads                      | M       | `checkInvariants` existiert bereits — es fehlt nur der Generator davor              |
...
**Reducer & Actions:** 1.3rest Invarianten‑Suite über alle Action‑Creator durch `gameReducer` ziehen und die Szenenwechsel‑Invariante ergänzen · 1.1b Dev‑Metrik für unbekannte Actions (identisch mit 8.2.4 — als ein Punkt umsetzen) · 8.2.1 `ActionPayloadMap` ableiten · 8.2.2 Snapshot‑Tests pro Action‑Creator
...
**7.8 Run Advisor Mode** ist im Audit als „Middleware‑Check vor Dispatch“ beschrieben. Das ist für sich genommen kein Verstoß gegen die `AGENTS.md`‑Regel: ein Check, der eine bereits erzeugte typisierte `GameAction` liest und sie durchlässt oder abbricht, ändert selbst keinen State und entmachtet den Reducer nicht. Die Regel wäre erst verletzt, wenn der Advisor Actions umschreibt oder eigene erzeugt. Umsetzung deshalb als reiner Regel‑Evaluator `(state, proposedAction) => AdvisorVerdict`; wo er aufgerufen wird — Hook‑Schicht oder Dispatch‑Wrapper — ist eine getrennte Entscheidung.

## Empfohlene Reihenfolge

1. **P0 komplett** — angeführt von 1.1a und 3.3rest (beides Einzeiler‑Klasse), dann der Persistenz‑Block 8.5.1/8.5.6/8.5.3.
2. **P1‑Fundament** in Tabellenreihenfolge — 5.1 zuerst, weil daran am meisten hängt; die S‑Punkte am Ende der Tabelle sind billige Mitnahmen.
