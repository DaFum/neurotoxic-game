...
1. [`roguelite-expedition/00a-exact-owner-contract-clarifications.md`](./roguelite-expedition/00a-exact-owner-contract-clarifications.md) — **narrow highest-priority clarifications that pin exact owners/files/effects for the Spec-Fidelity contract**
2. [`roguelite-expedition/00-spec-fidelity-execution-contract.md`](./roguelite-expedition/00-spec-fidelity-execution-contract.md) — restores the full approved gameplay scope and closes the remaining execution gaps
3. [`roguelite-expedition/00-review-hardening-contract.md`](./roguelite-expedition/00-review-hardening-contract.md) — mandatory reducer/trust-boundary/durability hardening wherever it does not conflict with the later Spec-Fidelity contracts
4. [`2026-09-03-roguelite-expedition-master-plan.md`](./2026-09-03-roguelite-expedition-master-plan.md)
5. [`roguelite-expedition/01-expedition-core-extraction.md`](./roguelite-expedition/01-expedition-core-extraction.md)
6. [`roguelite-expedition/02-condition-repairs-cargo.md`](./roguelite-expedition/02-condition-repairs-cargo.md)
7. [`roguelite-expedition/03-crew-stress-relationships.md`](./roguelite-expedition/03-crew-stress-relationships.md)
8. [`roguelite-expedition/04-pressure-rivals-contracts.md`](./roguelite-expedition/04-pressure-rivals-contracts.md)
9. [`roguelite-expedition/05-meta-regions-ascension.md`](./roguelite-expedition/05-meta-regions-ascension.md)
10. [`roguelite-expedition/06-balance-simulator-recalibration.md`](./roguelite-expedition/06-balance-simulator-recalibration.md)
...
2. `00a-exact-owner-contract-clarifications.md`
3. `00-spec-fidelity-execution-contract.md`
4. `00-review-hardening-contract.md`
5. the Masterplan
6. the numbered child plans

Rules:

- The **Exact Owner + Contract Clarifications** only tighten clauses explicitly named there: exact repository owners/test paths, persisted Sponsor semantics, map metadata validation, empty-Crew failure guard, exact Quest-event signatures, Brand Deal/Nemesis owner, Finale getter, exact Between-Tour effects, BandHQ files and exact Legendary run-state/routing. For those clauses it wins over all older plan text.
- The **Spec-Fidelity + Execution Contract is authoritative for every other contract it amends**. It restores the approved pre-tour build, route node classes, multi-axis failure model, active Condition/Crew consequences, Nemesis escalation, between-tour decisions, HQ transition and rule-changing Legendary rewards. If an older snippet conflicts with it, use the Spec-Fidelity replacement.
- The **Review Hardening Contract remains mandatory** for reducer authority, crash consistency, replay protection, persistence ordering, G6 horizon/cohort integrity and every other hardening rule compatible with the two later contracts.
- The Masterplan owns remaining cross-gate architecture, dependencies and stage gates.
- Each numbered child plan owns its subsystem tasks except where a contract above explicitly replaces/amends a step.
- Execute G1–G6 in numerical order. After G1, the Masterplan may still allow independent G2/G3 work, but each gate must apply its corresponding `Gx-Fy` Spec-Fidelity tasks, any named `C#` clarification, and the relevant `Gx-A/B/C` Hardening amendments before being green.
...
```bash
{
  cat docs/superpowers/plans/roguelite-expedition/00a-exact-owner-contract-clarifications.md
  printf '\n\n---\n\n'
