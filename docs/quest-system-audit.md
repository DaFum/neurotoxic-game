...
### 2.1 ✅ ERLEDIGT — `QuestCooldown.id` ist ein totes Feld

> **Fix:** Das Feld wurde vollständig entfernt — aus `QuestCooldown`/`QuestPenalty`
> (`quest.d.ts`), den Registry-Penalties, dem Penalty-Applier, dem Legacy-Normalizer
> und dem Load-Sanitizer. Cooldown-Matching läuft ausschließlich über `questId`.
...
### 2.5 ✅ ERLEDIGT — Doppelte/dreifache Flag-Mechanismen für dieselben Flags

> **Fix:** `completionFlags`/`failureFlags` sind jetzt die einzige Quelle; alle
> redundanten `flag.add`-Rewards/-Penalties wurden aus der Registry entfernt.
...
### 2.6 TEILWEISE ERLEDIGT — Tote Failure-Flags, kein Fail-Tracking

> **Update:** Die Failure-Flags sind konsolidiert (alle Story-Quests deklarieren
> `failureFlags`), und Retry-Cooldowns greifen seit Befund 1.3 auch für
> `'never'`-Quests. Ein `failedQuestIds`-Tracking existiert weiterhin nicht
> (bewusst: Fails sind wiederholbar, nur zeitverzögert).
