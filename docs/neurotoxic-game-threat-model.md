...
【F:src/main.jsx†L1-L11】【F:src/context/GameState.jsx†L369-L677】
...
- **Game state + persistence:** Game state is stored in memory and persisted to localStorage (`neurotoxic_v3_save`, `neurotoxic_global_settings`).【F:src/context/GameState.jsx†L369-L677】
...
- **React runtime → localStorage**
  ...
  - **Validation:** `safeStorageOperation` wraps storage operations in error handling; schema validation performed on load. (State validation logic in `loadGame`.)【F:src/context/GameState.jsx†L369-L677】
...
| localStorage persistence | Game save/load actions                      | Browser → Storage     | Attacker can alter localStorage to affect state | `loadGame`/`saveGame` in `GameState.jsx`【F:src/context/GameState.jsx†L369-L677】 |
