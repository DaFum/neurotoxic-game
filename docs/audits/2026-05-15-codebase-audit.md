...
The created action _always_ has a `payload`. The `if` is dead-safe but the `else` is unreachable. More importantly the entire dispatch is broken (see O4) — the action's payload is forwarded to `updateBand` where `delta` is a no-op field.
