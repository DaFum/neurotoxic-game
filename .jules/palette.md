...
## 2026-07-25 - Keyboard Accessibility for HUD Tooltips
...
**Action:** Avoid putting `tabIndex={0}` on non-interactive semantic wrappers like `div` inside Tooltips directly as it triggers lint errors (`noNoninteractiveTabindex`). Instead, refactor the `Tooltip` component to properly own the proxy wrapper and handle keyboard focus, or use inherently interactive semantic elements like `<button type="button">`.
