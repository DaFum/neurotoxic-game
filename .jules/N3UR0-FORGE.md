...
**Description:**
After travel, the band can acquire an item with special effects to use.
...
- State Impact:
  socialReducer.ts: zealotry metric (clamped 0-100).
  socialEngine.ts: Pure mathematical utilities calculating passive income vs. raid probability.
  src/data/brandDeals.ts: O(1) Map lookups enforcing requiresZealotry < 20 to filter available deals.
...
- State Impact:
  clinicReducer.ts: Added `handleClinicHeal` and `handleClinicEnhance` to strictly enforce stat bounds.
  initialState.ts: Tracking `clinicVisits` to exponentially increase cost over time.
  actionTypes.ts: Added `CLINIC_HEAL` and `CLINIC_ENHANCE`.
...
- State Impact:
  src/utils/imageGen.ts: Added new specific IMG_PROMPTS for social categories and brand alignments.
  src/components/postGig/SocialPhase.tsx: Integrates image generation into the background of post option buttons (opacity 10, screen blend mode).
  src/components/postGig/DealsPhase.tsx: Displays the generated brand alignment image side-by-side with the deal text.
  src/components/postGig/CompletePhase.tsx: Added full background outcome images for the gig success/failure screens.
...
- State Impact:
  actionTypes.ts: Added `PIRATE_BROADCAST`.
  socialReducer.ts: Added `handlePirateBroadcast` to apply bounds-clamped updates to player money, fame, band harmony, zealotry, and controversy.
  PirateRadioModal.tsx: A new Brutalist UI component added to the Overworld scene, allowing players to view the costs and gains and trigger the broadcast.
...
- State Impact:
  actionTypes.ts: Added `MERCH_PRESS`.
  socialReducer.ts: Added `handleMerchPress` enforcing bounds (money clamped >= 0, loyalty/controversy clamped 0-100, harmony clamped 1-100).
  useMerchPress.ts: Custom hook to calculate scaling costs and returns based on the band's current stats (fame multiplier).
  MerchPressModal.tsx: Brutalist UI utilizing flex-box chaining to present the brutal math of the merch hustle.
...
- State Impact:
  actionTypes.ts: Added `TRADE_VOID_ITEM`.
  tradeReducer.ts: New reducer strictly enforcing `fame` costs with `clampPlayerFame` and safely integrating into `gameReducer.ts`.
  BandHQ.tsx: Added `VoidTraderTab.tsx` integrated gracefully within the navigation, triggering only on `social.controversyLevel >= 30`. Uses Brutalist flex-box chaining.
...
- State Impact:
  actionTypes.ts: Added `BLOOD_BANK_DONATE`.
  clinicReducer.ts: Added `handleBloodBankDonate` enforcing bounds (money clamped >= 0, harmony clamped 1-100, stamina > 0).
  useBloodBank.ts: Custom hook to calculate dynamic scaling costs and manage modal.
  BloodBankModal.tsx: Brutalist UI utilizing flex-box chaining.

## 2026-04-11 - Amp Calibration Minigame

**Feature:** Amp Calibration
**Description:** Created a new minigame for PreGig phase alongside Roadie Run and Kabelsalat. The Amp Calibration minigame requires the player to match visual frequencies using a dial. It integrates with Tone.js for WebAudio integration and PixiJS for visual rendering (a waveform oscillating). Added core action creators, game state reducer logic (clamping scores, stress, harmony), hooked into SceneRouter, and modified the 1/3 RNG split in PreGig.tsx. Also implemented rigorous node tests and English/German translation files.
