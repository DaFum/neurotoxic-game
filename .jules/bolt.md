...
**Learning:** High-frequency trait unlock evaluations (like `Object.values().some` and `Array.some`) in `checkTraitUnlocks` allocate unnecessary closures and intermediate arrays. However, simply wrapping procedural loops in Immediately Invoked Function Expressions (IIFEs) inside array filter predicates or object iterators is a documented anti-pattern here that still allocates closures and hurts readability.
