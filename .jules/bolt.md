...
## 2026-08-07 - Mitigate array allocations from Object.entries
**Learning:** High-frequency usages of `Object.entries()` followed by `.map()` or inline iterations lead to `[key, value]` tuple array allocations and an intermediate array being mapped over, which increases Garbage Collection (GC) pressure unnecessarily. Converting them into an IIFE `(() => { const res = []; for (const k in obj) { if (!Object.hasOwn(obj, k)) continue; res.push(...) } return res; })()` eliminates the extra arrays while retaining declarative structure for React rendering.
**Action:** When finding `Object.entries()` usage inside performance paths, convert them into `for...in` paired with `Object.hasOwn()` if array allocation profiling calls for it, being careful with implicit `string` types indexing by using correct TypeScript casts.
