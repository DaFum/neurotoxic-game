...
2.  **Scan for References**
    ...
    ```bash
    .Codex/skills/asset-pipeline-verifier/scripts/asset-scan.sh
    ```

3.  **Verify Existence and Path**
    - For `import.meta.glob`: specific patterns (e.g., `/*.png`) must match files.
    - For `new URL`: Relative paths must be correct relative to the _source file_.
    - For `public/`: The path in code must _not_ include `/public`, just the content (e.g., `public/icon.ico` -> `/icon.ico`).
