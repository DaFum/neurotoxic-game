...
- **[HIGH]** `src/components/assets/sectionTabs.ts:13`: Component `AssetSectionTab` looks fully built but is never imported/rendered. **Action:** INTEGRATE or DELETE
...
- **[HIGH]** `src/components/assets/shared/ConfirmButton.tsx:7`: Interface/Type `ConfirmButtonProps` is exported but appears to be dead code (no imports found). **Action:** DELETE or remove export
