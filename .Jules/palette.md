...
**Learning:** Found that the main volume toggle (Mute/Unmute) button in the HUD was missing the `aria-pressed` attribute, despite having `aria-label` which updates based on state. Without `aria-pressed`, screen reader users might not know the toggle status of the button immediately.
**Action:** Always ensure that toggle buttons reflect their state using the `aria-pressed` attribute, in addition to context-aware `aria-label` updates.
